...
export async function POST(request: NextRequest) {
  ...
  let body: SerenaPageUpsertInput

  try {
    body = (await request.json()) as SerenaPageUpsertInput
  } catch {
    return NextResponse.json(
      {
        ok: false,
  ...
  payload.logger.info(
    `[serena] page ${existing ? 'updated' : 'created'} slug=${result.slug} status=${result._status} ip=${auth.clientIp || 'unknown'}`,
  )
